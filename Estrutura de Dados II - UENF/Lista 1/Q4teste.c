...
//Basicamente, esta função abre o arquivo original e, enquanto houver dados no arquivo , lê um valor do arquivo e armazena no vetor V. Sempre que o total de valores lidos for igual ao tamanho do vetor, os dados são ordenados usando a função quickSort e um novo arquivo temporário é gerado .
...
//Uma vez terminado o arquivo, verifica-se se não existem ainda dados no buffer (linha 69). Em caso afirmativo, um novo arquivo de dados ordenados é gerado Por fim, a função retorna o número de arquivos ordenados gerados ().
    if(total > 0){
        K++;
        sprintf(novo,"Temp%d.txt",K);
        qsort(V,total,sizeof(int),compara);
